...
try{
 ...
 console.log('Testing bulk student deletion');await login();await byName('นักเรียน').click();await page.getByLabel('เลือก ปุณณ์ สุขใจ').check();await page.getByLabel('เลือก ณิชา ใจดี').check();await byName('ลบที่เลือก (2)').click();await page.getByRole('heading',{name:'ลบนักเรียน 2 คน?'}).waitFor();await page.getByLabel('ยืนยันว่าตรวจรายชื่อแล้วและต้องการลบข้อมูลเหล่านี้').check();await byName('ยืนยันลบ 2 คน').click();await page.getByText('ลบนักเรียน 2 คนและข้อมูลที่เกี่ยวข้องแล้ว').waitFor();assert.equal(await page.getByLabel('เลือก ปุณณ์ สุขใจ').count(),0);assert.equal(await page.getByLabel('เลือก ณิชา ใจดี').count(),0);
 assert.deepEqual(errors,[]);console.log('PASS: self-registration → upload → approve → redeem → history → refund → adjustment → promotion → responsive → privacy timeout → bulk deletion; no browser errors');
