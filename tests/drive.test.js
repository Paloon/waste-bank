...
test("OAuth, private folder uploads, download, move and retention delete", async () => {
  ...
  const names = [
    ...
    "DRIVE_WASTE_FOLDER_ID",
    "DRIVE_REWARDS_FOLDER_ID",
  ];
  ...
  Object.assign(process.env, {
    ...
    DRIVE_EVIDENCE_FOLDER_ID: "evidence12345",
    DRIVE_WASTE_FOLDER_ID: "wastepicture12345",
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (String(url).includes("uploadType=multipart"))
      return Response.json({ id: "uploadedFile12345" });
    ...
    if (options.method === "PATCH")
      return Response.json({ id: "uploadedFile12345" });
    if (String(url).includes("uploadedFile12345"))
      return Response.json({ parents: ["evidence12345"] });
  ...
  try {
    ...
    const url = new URL(await drive.authUrl("teacher.mali"));
    assert.equal(
    ...
    assert.ok(
      seen
        .find((x) => x.url.includes("uploadType=multipart"))
        .options.body.includes("evidence12345"),
    );
    assert.equal(await (await drive.download(id)).text(), "photo");
    await drive.moveToWaste(id);
    assert.ok(
      seen
        .find((x) => x.options.method === "PATCH")
        .url.includes("addParents=wastepicture12345"),
    );
