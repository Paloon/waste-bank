...
export function act(s,actor,body){
 ...
 if(action==='submit'){
  ...
  requireStaff();fail(clean(p.name).length>=2,'กรอกชื่อรางวัล');let existing=s.rewards.find(x=>x.id===p.id);const before=existing?{...existing}:null;
  const incoming=p.images??(p.image?[{src:p.image}]:[]);fail(Array.isArray(incoming)&&incoming.length<=5,'รูปสินค้าได้สูงสุด 5 รูป');
  const images=incoming.map(entry=>{fail(entry&&typeof entry==='object','รูปภาพไม่ถูกต้อง');const zoom=Number(entry.zoom??1),x=Number(entry.x??0),y=Number(entry.y??0);fail(Number.isFinite(zoom)&&zoom>=1&&zoom<=3&&Number.isFinite(x)&&Math.abs(x)<=50&&Number.isFinite(y)&&Math.abs(y)<=50,'ตำแหน่งหรือการซูมรูปไม่ถูกต้อง');return {src:image(entry.src,true),zoom,x,y};});
  const oldImages=existing?.images?.length?existing.images.map(x=>x.src):existing?.image?[existing.image]:[];
  const removed=oldImages.filter(src=>src?.startsWith('drive:')&&!images.some(x=>x.src===src)).map(src=>src.slice(6));
  if(removed.length)s.pendingDriveDeletes=[...new Set([...(s.pendingDriveDeletes||[]),...removed])];
  const item={id:existing?.id||randomUUID(),name:clean(p.name,80),description:clean(p.description),price:integer(p.price,1),stock:integer(p.stock),enabled:!!p.enabled,icon:p.icon||'badge',color:'#e2ecd9',image:images[0]?.src||'',images};if(existing)Object.assign(existing,item);else s.rewards.push(item);log(item.id,before?{name:before.name,stock:before.stock,price:before.price}:null,{name:item.name,stock:item.stock,price:item.price});
